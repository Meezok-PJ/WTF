...
***
![22-47-17-WTF-24-08-2025.png](..\..\Finalized\WTF-images\22-47-17-WTF-24-08-2025.png)
***
...
For a deeper dive into the original project's architecture and design philosophy, check out the **[Automated Kali Docker Project](https://www.google.com/search?q=link-of-old-version-with-documentation)**. This is the foundation upon which the new, upgraded WTF tool was built.
...
ok Now lets Create Senario Use Case By testing Own Framework project
first we build docker image then Access it then we git clone our repo then we run the Tool 👍 treating my script as a Malicious github script 🏴‍☠️

Now we're talking. Let's showcase this sandbox in action. I'll add a new section that demonstrates a typical use case, treating your own framework as a real-world, potentially "malicious" tool.
...
![22-51-16-WTF-24-08-2025.png](..\..\Finalized\WTF-images\22-51-16-WTF-24-08-2025.png)
***
* **as u can see the Project/tool will be saved in home Directory path for easy access Project files later**
![22-53-46-WTF-24-08-2025.png](..\..\Finalized\WTF-images\22-53-46-WTF-24-08-2025.png)
* **After start the docker a the projects will built in directory specified  files + folders of Persistant mounted folder and the dockerfile/yaml file and readme  The when u run the optoin [1]**
![22-57-43-WTF-24-08-2025.png](..\..\Finalized\WTF-images\22-57-43-WTF-24-08-2025.png)
***
![23-02-50-WTF-24-08-2025.png](..\..\Finalized\WTF-images\23-02-50-WTF-24-08-2025.png)
***
...
![23-04-43-WTF-24-08-2025.png](..\..\Finalized\WTF-images\23-04-43-WTF-24-08-2025.png)
   * `[Output]` The terminal drops us into the isolated Kali environment. We are now inside the sandbox, ready to get to work. type `exit` to  be back to menu of kali enviroment 👇
   
   * ![23-27-54-WTF-24-08-2025.png](..\..\Finalized\WTF-images\23-27-54-WTF-24-08-2025.png)
- clone my tool to test use case example you are testing a script u found on github 
---
![19-26-36-WTF-25-08-2025.png](..\..\Finalized\WTF-images\19-26-36-WTF-25-08-2025.png)
---

As u can see in the Screenshot the u can debug a tool and see what it does or test how the code Operates  the example is a missing package called `bc`. 👇
![19-39-59-WTF-25-08-2025.png](..\..\Finalized\WTF-images\19-39-59-WTF-25-08-2025.png)

---
5.** We navigate to the `/mnt` directory (our persistent volume) that is connected to `~/sandbox` directory. use /mnt directory if u wanna mpve files/folder from contaner → to host Machine



6. **Run the Tool:** We run the tool's installation script, knowing it will install all its dependencies and create its own directories within the isolated container.
   ...
   * `[Output]` We see a flood of `apt-get install` commands and other installations, as the tool sets up its environment. All of this is contained and will not affect our host system.

7. **Exit and Confirm:** We exit the container with `exit`. The WTF menu reappears, confirming we are back on our host. We can then select `[2] Stop Kali` and then `[1] Start Kali` to prove that the environment and all our installed files are persistent.

   * `[Output]` When we access the shell again, we find the `/mnt/your-osint-framework` directory and all its contents exactly as we left them. The test is complete, and our host system remains clean.

This simple workflow demonstrates the core value of the **WTF Kali Sandbox**: a controlled environment for every chaotic GitHub tool you might encounter.



Perfect — you want the image links **kept in the README but commented out** so the markdown doesn’t render them, right?\
That way the paths stay organized/documented, but the images won’t clutter the page.

Here’s the cleaned-up README with all images converted into commented Markdown:

***

Got it ⚡— you want every image reference **kept in place** but **commented out** so it doesn’t render.\
I’ll rewrite your README with `<!-- ... -->` wrapping all the `![image]()` lines.
...
You don’t want them hidden (`<!-- ... -->`), you want them as **visible placeholders** like:

```markdown
[image](..\..\Finalized\WTF-images\22-47-17-WTF-24-08-2025.png)
```

Here’s your draft with all the `![]()` converted to `[image]()` so you can drop it into Zettlr and still have your screenshots as easy-to-copy references 👇
...
---
![image](..\..\Finalized\WTF-images\22-51-16-WTF-24-08-2025.png)  
---

* **As you can see the project/tool will be saved in the home directory path for easy access later**
---
![image](..\..\Finalized\WTF-images\22-53-46-WTF-24-08-2025.png)  
---
* **After starting Docker, the project is built in the specified directory with persistent storage, Dockerfile/yaml, and README**

![image](..\..\Finalized\WTF-images\22-57-43-WTF-24-08-2025.png)  

---

![image](..\..\Finalized\WTF-images\23-02-50-WTF-24-08-2025.png)  
...
![image](..\..\Finalized\WTF-images\23-04-43-WTF-24-08-2025.png)  
---
...
![20-29-33-WTF-25-08-2025.png](..\..\Finalized\WTF-images\20-29-33-WTF-25-08-2025.png)
...
![20-43-49-WTF-25-08-2025.png](..\..\Finalized\WTF-images\20-43-49-WTF-25-08-2025.png)
